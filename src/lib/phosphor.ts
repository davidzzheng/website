/**
 * GPU-side morphing particle system.
 * All per-particle math (morph lerp, curl noise, mouse repulsion) runs
 * in the vertex shader — CPU only updates ~4 uniforms per frame.
 */
...
const PARTICLE_COUNT = window.innerWidth < 768 ? 1200 : 3000;

// ── Target shape generators (unchanged) ──

function textTargets(text: string): Float32Array {
  const off = document.createElement("canvas");
  off.width = 1200; off.height = 400;
  const ctx = off.getContext("2d")!;
  ctx.fillStyle = "#fff";
  ctx.font = `bold ${window.innerWidth < 768 ? 100 : 160}px "Geist Variable", sans-serif`;
  ...
  for (let y = 0; y < 400; y += 3) {
    for (let x = 0; x < 1200; x += 3) {
      if (data[(y * 1200 + x) * 4 + 3]! > 128) pixels.push([(x - 600) * 0.8, (200 - y) * 0.8]);
    }
  }
  const arr = new Float32Array(PARTICLE_COUNT * 3);
  for (let i = 0; i < PARTICLE_COUNT; i++) {
    const px = pixels[Math.floor(Math.random() * pixels.length)]!;
    arr[i * 3] = px[0] + (Math.random() - 0.5) * 4;
    arr[i * 3 + 1] = px[1] + (Math.random() - 0.5) * 4;
...
const LOGO_PATHS: Record<string, string> = {
  ...
  "AssistList": "M2440 5703 c-371 -68 -597 -141 -880 -284 -564 -285 -1008 -739 -1286 -1314 -128 -265 -212 -537 -253 -821 -30 -201 -30 -647 0 -848 68 -464 260 -924 543 -1299 428 -568 1036 -952 1732 -1092 118 -24 133 -25 565 -25 l444 0 140 29 c689 143 1295 529 1718 1095 245 326 419 712 513 1136 27 123 27 132 31 530 5 429 2 472 -44 680 -89 409 -294 834 -564 1170 -103 129 -302 329 -429 432 -349 281 -769 481 -1220 580 l-145 32 -420 2 c-231 1 -431 0 -445 -3z m-407 -1238 c213 -37 421 -167 554 -345 50 -66 103 -171 133 -263 l22 -67 31 26 c18 14 90 79 161 143 424 380 526 450 734 497 91 22 258 23 347 5 338 -73 593 -315 681 -648 26 -100 24 -310 -5 -418 -92 -350 -384 -609 -739 -656 -259 -34 -534 61 -723 250 -91 91 -139 166 -139 214 0 43 8 52 188 222 117 111 137 117 157 48 41 -137 175 -258 324 -293 130 -31 288 18 389 119 122 122 159 290 102 457 -62 178 -251 297 -449 281 -127 -10 -154 -30 -767 -577 -306 -272 -577 -512 -603 -533 -74 -61 -172 -114 -272 -149 -82 -28 -110 -32 -227 -36 -236 -8 -406 44 -586 180 -143 107 -264 297 -306 481 -30 129 -25 342 10 452 98 310 349 537 670 605 106 23 202 24 313 5z m1028 -1710 c314 -63 573 -324 656 -661 23 -96 23 -296 -1 -389 -54 -208 -192 -401 -377 -524 -148 -100 -287 -141 -473 -141 -147 0 -255 23 -371 80 -138 66 -275 186 -356 310 -73 112 -696 1195 -689 1198 4 1 45 -8 91 -21 110 -32 198 -46 339 -55 l115 -7 51 -90 c28 -49 56 -91 61 -92 5 -2 37 32 72 76 214 272 531 386 882 316z ||| M1790 4026 c-152 -33 -277 -146 -325 -295 -13 -40 -16 -76 -13 -146 3 -82 8 -101 41 -168 63 -128 175 -211 319 -238 227 -42 457 126 498 364 31 175 -67 361 -233 442 -109 53 -184 64 -287 41z ||| M2765 2326 c-129 -31 -241 -122 -298 -243 -30 -64 -32 -75 -32 -178 0 -105 2 -113 34 -180 86 -175 272 -276 453 -246 203 33 352 193 365 391 13 181 -76 339 -235 419 -98 49 -189 61 -287 37z",
...
export function initFlowField(canvas: HTMLCanvasElement): () => void {
  ...
  const targetShapes = [nebulaTargets(), waveTargets(), vortexTargets(), dissolveTargets()];

  // ── Buffers: start/target positions for GPU morphing ──
  const startPositions = new Float32Array(PARTICLE_COUNT * 3);
  const targetPositions = new Float32Array(PARTICLE_COUNT * 3);
  startPositions.set(targetShapes[0]!);
  targetPositions.set(targetShapes[0]!);
  ...
  const material = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uMorphProgress: { value: 1 }, // start fully morphed (at target)
    ...
    vertexShader: `
      ...
      uniform vec2 uMouse;
      uniform float uRotCY;
      uniform float uRotSY;
      uniform float uRotCX;
      uniform float uRotSX;

      varying float vDepth;
      varying float vRandom;

      void main() {
        // ── GPU morph: smoothstep-eased lerp from start to target ──
        float t = uMorphProgress;
        t = t * t * (3.0 - 2.0 * t); // smoothstep
        vec3 morphed = mix(aStart, aTarget, t);

        // ── Curl noise perturbation (organic flow) ──
        float nX = sin(morphed.y * 0.018 + uTime * 0.5) * cos(morphed.z * 0.014 + uTime * 0.35) * 1.2;
        float nY = sin(morphed.z * 0.018 + uTime * 0.4) * cos(morphed.x * 0.014 + uTime * 0.3) * 1.2;
        float nZ = sin(morphed.x * 0.018 + uTime * 0.3) * cos(morphed.y * 0.014 + uTime * 0.45) * 0.8;
        vec3 noised = morphed + vec3(nX, nY, nZ);
        vec3 rotated;
        rotated.x = noised.x * uRotCY - noised.z * uRotSY;
        rotated.z = noised.x * uRotSY + noised.z * uRotCY;
        rotated.y = noised.y * uRotCX - rotated.z * uRotSX;
        rotated.z = noised.y * uRotSX + rotated.z * uRotCX;

        // Breathing
        float breath = sin(uTime * 0.5 + aRandom * 6.28) * 0.03;
        vec3 finalPos = rotated * (1.0 + breath);

        // Transform to view space — XY here corresponds to screen position
        vec4 mv = modelViewMatrix * vec4(finalPos, 1.0);

        vec2 mouseVS = vec2(uMouse.x * 650.0, -uMouse.y * 380.0);
        vec2 toMouse = mv.xy - mouseVS;
        float distSq = dot(toMouse, toMouse);
        if (distSq < 40000.0 && distSq > 1.0) {
          float force = (1.0 - distSq / 40000.0) * 6.0;
          mv.xy += toMouse * inversesqrt(distSq) * force;
        }
  ...
  // Bloom — gives text + particles unified glow
  const composer = new EffectComposer(renderer);
  composer.addPass(new RenderPass(scene, camera));
  const bloomPass = new UnrealBloomPass(new THREE.Vector2(w, h), 0.15, 0.3, 0.35);
  ...
  // ── Company logo morphing (SVG path → particle targets) ──
  ...
  function svgPathToTargets(pathData: string): Float32Array {
    ...
    if (pathData.includes("|||")) {
      for (const frag of pathData.split("|||")) {
        const p = document.createElementNS(svgNS, "path");
        p.setAttribute("d", frag.trim());
        svg.appendChild(p);
        allPaths.push(p);
      }
    } else {
    ...
    const SAMPLE = 400;
    const totalLen = allPaths.reduce((s, p) => s + p.getTotalLength(), 0);
    const samples: number[] = [];
    allPaths.forEach((pathEl) => {
      const ctm = pathEl.getCTM();
      const len = pathEl.getTotalLength();
      const count = Math.max(5, Math.round((len / totalLen) * SAMPLE));
  ...
  function onHover(e: Event) {
    ...
    if (!detail) { hoverTarget = null; hoveredCompany = ""; return; }
    if (detail.startsWith("__text__:")) {
      const text = detail.slice(8);
      if (!companyCache.has(detail)) companyCache.set(detail, textTargets(text));
      hoverTarget = companyCache.get(detail)!;
      hoveredCompany = detail;
    } else if (LOGO_PATHS[detail]) {
  ...
  // ── Target switching — updates aStart/aTarget, resets progress ──
  let lastTargetKey = "";

  function switchTarget(newTarget: Float32Array) {
    // Bake current morphed positions into aStart
    const progress = material.uniforms.uMorphProgress.value;
    const t = progress * progress * (3 - 2 * progress); // smoothstep
    for (let i = 0; i < PARTICLE_COUNT * 3; i++) {
      startPositions[i] = startPositions[i]! * (1 - t) + targetPositions[i]! * t;
    }
    // Set new target
    targetPositions.set(newTarget);
    // Reset progress
    material.uniforms.uMorphProgress.value = 0;
    // Flag GPU buffers for re-upload (one-time cost, not per-frame)
    geometry.attributes.aStart.needsUpdate = true;
    geometry.attributes.aTarget.needsUpdate = true;
  }

  // ── Animation loop — GPU does all per-particle work ──
  let raf = 0, t = 0, alive = true, paused = false;
  ...
  function draw() {
    ...
    // Accumulate morph progress toward 1
    if (material.uniforms.uMorphProgress.value < 1) {
      material.uniforms.uMorphProgress.value = Math.min(1, material.uniforms.uMorphProgress.value + (hoverTarget ? 0.024 : 0.012));
    }

    // Check for target changes
    const targetArr = hoverTarget ?? targetShapes[Math.min(targetShapes.length - 1, getActiveSection())]!;
    const targetKey = hoverTarget ? `hover:${hoveredCompany}` : String(getActiveSection());
    if (targetKey !== lastTargetKey) {
      lastTargetKey = targetKey;
      switchTarget(targetArr);
    }

    const rotY = t * (hoverTarget ? 0.02 : 0.06) + mx * 0.2;
    const rotX = t * 0.03 + my * 0.15;
    material.uniforms.uRotCY.value = Math.cos(rotY);
    material.uniforms.uRotSY.value = Math.sin(rotY);
    material.uniforms.uRotCX.value = Math.cos(rotX);
    material.uniforms.uRotSX.value = Math.sin(rotX);
